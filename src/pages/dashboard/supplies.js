...
const Supplies = () => {
  ...
  const url = `https://cottage-backend-voilerplate.vercel.app/helpDesk/inquiry/requestSupply?page=${page}&size=${size}`;

  const { data, isLoading, refetch, isError } = useQuery({
    queryKey: ["requestSupply", page, size],
    queryFn: async () => {
    ...
    <div className="border-t-2">
      <div className="overflow-x-auto">
        <table className="table w-full border-separate border-spacing-y-3">
          <thead>
            <tr className="bg-gray-100 dark:bg-slate-800 dark:text-gray-200 text-center">
              {[
                "Serial",
                "Name",
                "Phone",
                ...
                <th key={index} className="border border-gray-300 py-3">
                  {header}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {allInfo.map((user, index) => (
              <tr
                key={user._id}
                className="hover dark:bg-slate-500 dark:text-gray-100  border-gray-300 border"
              >
                <td className="border py-2">{index + 1}</td>
                <td className="border py-2">{user?.name || "N/A"}</td>
                <td className="border py-2">{user?.phone || "No data"}</td>
                <td className="text-[#00A6B2] border py-2">
                  {user?.employeeId}
                </td>
                <td className="border">
                  <button
                    onClick={() => openModal(user?.subject)}
                    className="text-sm bg-[#00A6B2] py-2 px-3 rounded-md text-white shadow-lg cursor-pointer"
                  >
                    See Message
                  </button>
                </td>
                <td className="text-[#00A6B2] text-sm font-medium border py-2">
                ...
                <td className="border py-2">
                  <button
                    ...
                    className="btn btn-sm bg-red-600 uppercase text-white px-2 py-1.5 rounded-md text-xs"
                  >
                    Delete
                  </button>
                </td>
