...
export async function getStaticProps({ params }) {
  try {
    const res = await fetch(
      `http://localhost:5000/blogs/single/${params.slug}`
    );

    if (!res.ok) {
      return { notFound: true }; // ✅ Handle API errors
    }

    const blog = await res.json();

    if (!blog || Object.keys(blog).length === 0) {
      return { notFound: true }; // ✅ Ensure blog exists
    }

    return {
      props: { blog }, // ❌ No ISR, pure SSG
    };
  } catch (error) {
...
export async function getStaticPaths() {
  try {
    ...
    return {
      paths,
      fallback: false, // ❌ No fallback, Pure SSG
    };
  } catch (error) {
            ...
            <div className="border-[1.5px] min-h-[30rem] lg:w-[90%] w-[95%] mx-auto border-t-primary hidden md:block">
              ...
              <div className="mx-auto px-2 text-lg mt-3 text-center w-[80%] text-[#6c6262]">
                <p className="font-medium dark:text-gray-100 text-[16px] leading-relaxed open-sans">
                  ...
                  for You . HHA/PCA, NHTD, Private PAY & CDPAP Services
              ...
              <div className="text-center mt-4">
                <Link href="/">
                  <button className="bg-gray-50 px-3 py-3 shadow-md text-lg text-primary uppercase font-semibold bg-hov2 league-spartan">
                    Read More
                  </button>
                </Link>
              </div>
